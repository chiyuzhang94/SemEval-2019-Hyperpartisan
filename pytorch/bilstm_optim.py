...
for item in op_parameters:
    ...
    for value in item['var_range']:
        ...
        if cuda_gpu:
            bilstm = LSTM(int(embedding_dim),int(num_layers),int(hidden_dim),int(batch_size),dropout,cuda_gpu).cuda()
        else:
            bilstm = LSTM(int(embedding_dim),int(num_layers),int(hidden_dim),int(batch_size),dropout,cuda_gpu)
...
with open('./output/bilstm/optimize_bilstm.json', 'w') as outfile2:
