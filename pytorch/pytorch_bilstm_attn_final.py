...
load_model = True
load_model_path = "./output/bilstmattn/pytorch_bilstmattn_4"
...
num_epoch = 5  # The number of iteration
...
if load_model:
    ...
    with open('./output/bilstmattn/lstmattn_tokenizer.pickle', 'rb') as handle:
...
with open('./output/bilstmattn/lstmattn_tokenizer.pickle', 'wb') as handle:
...
training_loss, training_acc, test_acc =  train.train_with_w2v(num_epoch,train_size,batch_size,optimizer,X_train,y_train,sequence_length,embedding_dim,embedding_matrix,lstmattn,test_size,loss_func,X_test,y_test,"bilstmattn",batch_first,cuda_gpu)

end = time()
print("time",end - start)

with open('./output/bilstmattn/train_loss_lstmattn.json', 'w') as outfile1:
    json.dump(training_loss, outfile1)  # write training loss results to a json file

with open('./output/bilstmattn/test_accuracy_lstmattn.json', 'w') as outfile2:
    json.dump(test_acc, outfile2) # write test accuracy results to a json file

with open('./output/bilstmattn/train_accuracy_lstmattn.json', 'w') as outfile3:
