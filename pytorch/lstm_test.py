...
load_model = True
load_model_path = "./output/lstm/pytorch_lstm_3"
...
hidden_dim = 300 # The number of unit in a hidden layer
...
if cuda_gpu:
    ...
    lstm.load_state_dict(torch.load(load_model_path, map_location='cpu'))
print(lstm)
...
acc,output_dic = test.test_with_w2v(test_set,test_label,data_size,vocabulary_size,sequence_length,load_model,tokenizer,batch_size,embedding_dim,embedding_matrix,lstm,batch_first)
print("accuracy",acc)
# store results
with open('./output/lstm/test_output_lstm.json', 'w') as outfile2:
